...
async function loadStored() {
  if (redis) return (await redis.get("elon-posts")) ?? [];
  try {
...
async function savePosts(posts) {
  if (redis) { await redis.set("elon-posts", posts); return; }
...
/**
 * Load stored posts, fetch any new ones from xtracker,
 * merge, save, and return all posts sorted chronologically.
 *
 * Returns { posts, newCount, totalCount, lastPostAt }
 */
export async function syncPosts({ forceLookbackDays = 0 } = {}) {
  ...
  // Force re-fetch by rolling the window back so we pick up any missed posts
  if (forceLookbackDays > 0 && newestDate) {
    const lookback = new Date(Date.now() - forceLookbackDays * 24 * 60 * 60 * 1000).toISOString();
    if (lookback < newestDate) newestDate = lookback;
  }
